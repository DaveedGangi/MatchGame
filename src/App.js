...
import ListOfTheImages from './SendingElements/sendingTheImages'
import './App.css'
...
class App extends Component {
  state = {
    selectingUpperImage: imagesList[0].imageUrl,
    TabListsAll: tabsList,
    category: 'FRUIT',
  ...
  statusChange = () => {
    ...
    if (time !== 0) {
      ...
      this.setState({conditionWill: true})
    }
  }
  ...
  changingUpperImg = imageUrl => {
    const {score, selectingUpperImage} = this.state
    if (imageUrl === selectingUpperImage) {
      this.setState({score: score + 1})
    } else {
      this.setState({conditionWill: true, specifying: 'match'})
      clearInterval(this.intervalId)
    }
    this.setState({
      selectingUpperImage:
        imagesList[Math.ceil(Math.random() * imagesList.length - 1)].imageUrl,
      specifying: selectingUpperImage,
    })
  }
  ...
  render() {
    ...
    console.log(selectingUpperImage)
    console.log(TabListsAll)

    const SelectedImages = ImagesAll.filter(each => each.category === category)

    return conditionWill ? (
      <div className="bgForAll">
        <div>
          <nav className="Nav">
            <div>
              <img
            ...
            <ul className="timeSide">
              <li>
                <p>Score:{score}</p>
              </li>
              <li>
                <p className="TimeShown">
                  <img
                    className="TimerLogo"
        ...
        <div className="result">
          <div className="resultCardImage">
            ...
            <div>
              <button
                onClick={this.ResetToFirst}
                className="playAgainButton"
                type="button"
              >
                <div className="ResetLog">
                  <img
                    className="resetLogo"
      ...
      <div className="bgForAll">
        ...
        <div className="upperSideImage">
          <ul>
            <li key={selectingUpperImage}>
              <img
                className="upperImage"
                src={selectingUpperImage}
                alt={specifying}
              />
              console.log(specifying)
            </li>
          </ul>
        </div>
        ...
        <div className="ButtonBg">
          <ul className="UlButton">
            <li>
              <button onClick={this.ChangeToFruit} type="button">
                Fruits
              </button>
            </li>
            <li>
              <button onClick={this.ChangeToAnimals} type="button">
                Animals
              </button>
            </li>
            <li>
              <button onClick={this.ChangeToPlaces} type="button">
                Places
              </button>
            </li>
          </ul>
        ...
        <div className="ImageCard">
          <ul className="Ul">
            {SelectedImages.map(each => (
              <ListOfTheImages
                eachElement={each}
                key={each.id}
                changeTheImage={this.changingUpperImg}
              />
            ))}
          </ul>
        </div>
